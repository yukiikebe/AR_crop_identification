...
def tif_to_rgb(tif_path: Path,
    ...
    if stats_counter is not None:
        ...
        for cid, c in zip(unique, cnts):
            if cid <= 0 or cid not in colormaps:   # non‑selected / unknown
                continue
            else:
...
def main():
    ...
    # Load first tile to know dimensions after thumb --------------------- #
    sample_img = tif_to_rgb(next(iter(tiles.values())), remap_table, args.thumb)
    ...
    for (x, y), tif_path in tqdm(tiles.items()):
        img = tif_to_rgb(tif_path, remap_table, args.thumb, pixel_counts)
        # flip y so row 0 is at top
        top_left = (x * tile_w, (max_y - y) * tile_h)
        mosaic.paste(img, top_left)

    # --------------------------------------------------------------------- #
    # Legend + row/column indices                                           #
    # --------------------------------------------------------------------- #
    present_ids = sorted(class_names.keys())

    # Re‑use the font object (create if it doesn’t exist yet)
    try:
        font = ImageFont.truetype("DejaVuSans.ttf", size=max(tile_h // 5, 10))
    except OSError:
        font = ImageFont.load_default()

    legend_img = build_legend(present_ids, tile_h, font)

    # ---------- build Y‑axis (row) labels --------------------------------- #
    y_label_w = max(60, tile_w // 3)          # gutter width on the left
    y_label_img = Image.new("RGB", (y_label_w, mosaic_h), (255, 255, 255))
    draw_y = ImageDraw.Draw(y_label_img)

    # --- Y-axis labels ---
    for y in range(max_y + 1):
        y_pix = (max_y - y) * tile_h + tile_h // 2
        txt   = str(y)
        tw, th = get_text_size(draw_y, txt, font)
        draw_y.text((y_label_w - tw - 4, y_pix - th // 2),
                    txt, fill="black", font=font)

    # ---------- build X‑axis (column) labels ------------------------------ #
    x_label_h = max(40, tile_h // 3)          # gutter height at the bottom
    x_label_img = Image.new("RGB", (mosaic_w, x_label_h), (255, 255, 255))
    draw_x = ImageDraw.Draw(x_label_img)

    # --- X-axis labels ---
    for x in range(max_x + 1):
        x_pix = x * tile_w + tile_w // 2
        txt   = str(x)
        tw, th = get_text_size(draw_x, txt, font)
        draw_x.text((x_pix - tw // 2, 4), txt, fill="black", font=font)

    # ---------- assemble the final canvas --------------------------------- #
    final_w = y_label_w + mosaic_w + legend_img.width
    final_h = mosaic_h + x_label_h
    final   = Image.new("RGB", (final_w, final_h), (255, 255, 255))

    final.paste(y_label_img, (0,                     0))          # left gutter
    final.paste(mosaic,       (y_label_w,            0))          # main mosaic
    final.paste(legend_img,   (y_label_w + mosaic_w, 0))          # legend
    final.paste(x_label_img,  (y_label_w,            mosaic_h))   # bottom gutter
    ...
    # --------------------------------------------------------------------- #
    pixel_counts = {cid: pixel_counts[cid] for cid in present_ids if cid in pixel_counts}
    pixel_counts.pop(1)
    if pixel_counts:
        ...
        print("––  –––––––––––––––––––––––––––  –––––––––  –––––––––––")
        for cid in sorted(k for k in pixel_counts if k != 0):
