...
def resize_all_bands_to_wh(sat: dict) -> dict:
    """Resize each band from shape (H, W) to (W, H) using nearest interpolation."""
    for res in sat:
        for b in sat[res]:
            h, w = sat[res][b].shape
            sat[res][b] = cv2.resize(sat[res][b], (h, w), interpolation=cv2.INTER_NEAREST)
    return sat
...
def load_meta_patch(meta_dir: str, remap_table: dict[int, int]):
    ...
    for d in sorted(dates):
        ...
        if sat is None:
            continue
        sat = resize_all_bands_to_wh(sat)  # Ensure all bands are resized to (W, H) due to a bug in downloading code
        stack_sat = stack_bands(sat)
        stacks.append(np.concatenate([stack_sat, cdl_exp], axis=-1))
...
# ──────────────────── CDL REMAPPING HELPERS ──────────────────────
def load_remap_table(path: str) -> dict[int, int]:
    """Read the class‑map JSON produced by your preprocessing step."""
    with open(path) as fh:
        raw = json.load(fh)
    return {int(k): int(v["remapped_id"]) for k, v in raw.items()}

def remap_cdl(cdl: np.ndarray, table: dict[int, int]) -> np.ndarray:
    """Return a uint8 array with values remapped (unknown→0)."""
    out = np.zeros_like(cdl, dtype=np.uint8)
...
def preprocess_sub_region(sub_dir: str, out_root: str):
    ...
    for tile, (iy, ix) in zip(tiles, ids):
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser("California S2 → Blosc2 frame converter (uint16)")
